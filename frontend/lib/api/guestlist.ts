 ...
 */
export async function getGuestList(eventId: string): Promise<GuestListResponse> {
 ...
 * Add a new guest
 */
export async function addGuest(eventId: string, guestData: CreateGuestData): Promise<Guest> {
  const response = await fetch(`${API_BASE_URL}/events/${eventId}/guests`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
 ...
 */
export async function sendInvitation(eventId: string, guestId: string): Promise<{ message: string; sentTo: string }> {
